...
class RTGM_Util {
   ...
   *
   * Note :: X-values must be monotonically increasing for this to work. See
   *         also "findLogLogY_NonMonotonic" if the x-values do not meet this
   *         requirement.
   *
  ...
  /**
   * Returns the log-log interpoloated or extrapolated y-value using the
   * supplied x- and y-value arrays.
   *
   * Note :: This method is slower than the regular "findLogLogY" method,
   *         however this method also works for non-monotonically increasing
   *         x-values.
   *
   * @param xs x-values of some function
   * @param ys y-values of some function
   ...
   * @return the log-log interpolated y-value
   */
  public static function findLogLogY_NonMonotonic ($xs, $ys, $x) {
    $lower = RTGM_Util::lowerBound($xs, $x);
    $upper = RTGM_Util::upperBound($xs, $x);

    return exp(RTGM_Util::findY(log($xs[$lower]), log($ys[$lower]),
        log($xs[$upper]), log($ys[$upper]), log($x)));
  }
...
//    $i = ($i == -1) ? 0 : ($i < 0) ? -$i - 2 : $i;
  ...
  /**
   * @param $values {Array}
   *      An array containing numbers to search.
   * @param target {Number}
   *      The target for which to search.
   *
   * @return {Number}
   *      The index of the larget value found in the $values array that is
   *      smaller than the $target value. If no such value is found, 0 is
   *      returned.
   */
  private static function lowerBound ($values, $target) {
    $index = null;
    $size = count($values);
    ...
    for ($i = 0; $i < $size; $i++) {
      ...
      if ($value < $target && ($index === null || $value > $values[$index])) {
        $index = $i;
      }
    }
   ...
   *
   * @return {Number}
   *      The index of the smallest value found in the $values array that is
   *      larger than the $target value. If no such value is found,
   *      count($values) - 1 is returned.
   */
  private static function upperBound ($values, $target) {
    $index = null;
    $size = count($values);
